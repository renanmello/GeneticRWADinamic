...
def optimize_hyperparameters(n_trials: int = 100) -> Dict[str, any]:
    ...
    best_results = {
        "best_params_AG_algorithm": best_params,
        "best_value_AG_algorithm": study.best_value,
        "datetime": str(datetime.datetime.now())
    }

    output_path = f"best_params_AG_algorithm_{GLOBAL_WAVELENGTHS}_wavelengths.json"
    with open(output_path, "w") as f:
        json.dump(best_results, f, indent=4)

    print(f"Resultados AG algorithm salvos em {output_path}")
...
def main():
    ...
    for num_wavelengths in [4,8]:
        ...
        optimization_results = optimize_hyperparameters(n_trials=50)
        ...
        # Executa simulação
        results, best_individual, best_fitness_value = wdm_simulator.simulate_network(num_simulations=5, output_file=f"blocking_results_GA_{GLOBAL_WAVELENGTHS}_wavelengths.txt")  # numero de simulações

        # Gera visualizações e relatórios
        wdm_simulator.plot_individual_genes(save_path=f"grafico_wdm_simulation_GA_{GLOBAL_WAVELENGTHS}_wavelengths.png")
        wdm_simulator.generate_comparison_table(output_file=f"comparacao_rotas_GA_{GLOBAL_WAVELENGTHS}_wavelengths.csv")
