            ...
            for run in range(num_runs):
                ...
                for req_idx, (source, target) in enumerate(self.requests):
                    ...
                    for call_idx in range(total_calls_per_load):
                        # Incrementa tempo simulado
                        self.current_time += 0.1  # Pequeno incremento de tempo
...
def main():
    ...
    # Usaremos 1.0 a 5.0 unidades de tempo como exemplo
    results = simulator.simulate_traffic_multiple_runs(
        num_runs=10,
        total_calls_per_load=1000,
        call_duration_range=(1.0, 5.0)  # Duração das chamadas entre 1.0 e 5.0
    )
